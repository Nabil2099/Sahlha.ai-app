...
class SahlhaAppBar extends StatelessWidget implements PreferredSizeWidget {
  const SahlhaAppBar({super.key, required this.title, this.actions});
...
class MasteryRing extends StatelessWidget {
  const MasteryRing(
      {super.key, required this.mastered, required this.total, this.size = 96});
        ...
        children: [
          SizedBox(
            ...
            child: CircularProgressIndicator(
              ...
              backgroundColor: SahlhaColors.tealSoft,
              valueColor:
                  const AlwaysStoppedAnimation<Color>(SahlhaColors.teal),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          color: bg, borderRadius: BorderRadius.circular(SahlhaRadius.pill)),
      child: Text(masteryLabel(state),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.symmetric(
            horizontal: SahlhaSpacing.lg, vertical: SahlhaSpacing.lg),
        decoration: BoxDecoration(
          color: bg,
          borderRadius: BorderRadius.circular(SahlhaRadius.md),
...
class InlineFeedback extends StatelessWidget {
  const InlineFeedback(
      {super.key, required this.correct, required this.message});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          color: bg, borderRadius: BorderRadius.circular(SahlhaRadius.md)),
      child: Row(
          ...
          children: [
            Container(
              ...
              decoration: const BoxDecoration(
                  color: SahlhaColors.tealSoft, shape: BoxShape.circle),
              child: const Icon(Icons.auto_stories_outlined,
                  color: SahlhaColors.teal, size: 34),
            ),
            const SizedBox(height: SahlhaSpacing.lg),
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.cloud_off_outlined,
                color: SahlhaColors.muted, size: 40),
            const SizedBox(height: SahlhaSpacing.lg),
            Text(message,
...
class SahlhaLogo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        SvgPicture.asset('assets/logo/sahlha_mark.svg',
    ...
    switch (state) {
      ...
      case PathNodeState.current:
        ...
        mark = Container(
            ...
            height: 18,
            decoration: const BoxDecoration(
                color: SahlhaColors.sun, shape: BoxShape.circle));
      case PathNodeState.upcoming:
        ring = SahlhaColors.line;
        fill = Colors.white;
        mark = const Icon(Icons.circle_outlined,
            color: SahlhaColors.muted, size: 18);
      case PathNodeState.locked:
        ring = SahlhaColors.line;
        fill = const Color(0xFFF1F5F9);
        mark = const Icon(Icons.lock_outline,
            color: SahlhaColors.muted, size: 18);
    }
    final dimmed =
