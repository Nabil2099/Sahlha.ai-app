          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(user?.name.split(' ').first ?? 'Parent',
                  ...
                  data: (list) {
                    if (list.isEmpty) {
                      return EmptyState(
                        ...
                        message:
                            'Ask your child for their parent link code (they find it in Profile), then link them here.',
...
class _HomeBody extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final text = Theme.of(context).textTheme;
    final children =
        ref.watch(linkedChildrenProvider).valueOrNull ?? [];
      ...
      children: [
        ...
        progress.when(
          ...
          error: (e, _) => ErrorState(
              message: e.toString(),
              onRetry: () =>
                  ref.invalidate(selectedChildProgressProvider)),
          data: (data) {
            final rooms =
                (data['classrooms'] as List? ?? []);
            if (rooms.isEmpty) {
              return const SahlhaCard(
                  child: Text(
                      'Your child has not joined a classroom yet.'));
            }
            return Column(
              children: rooms.map((r) {
                ...
                final summary = (room['summary'] as Map?)
                    ...
                    {};
                int count(String k) =>
                    (summary[k] as num?)?.toInt() ?? 0;
                final total = count('mastered') +
                    count('developing') +
                    count('needs_practice') +
                    count('not_started');
                final recent =
                    (room['recent_score'] as num?)?.toDouble();
                return Padding(
                  padding: const EdgeInsets.only(
                      bottom: SahlhaSpacing.md),
                  child: SahlhaCard(
                    onTap: () => context.push(
                        '/parent/children/${selected.id}'),
                    child: Column(
                      crossAxisAlignment:
