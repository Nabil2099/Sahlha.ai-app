      ...
      data: (list) {
        if (list.isEmpty) {
          return const EmptyState(
            ...
            message: 'Share the classroom join code (Materials tab shows it too) so students can join.',
          );
        }
        ...
        for (final s in (mastery.value?['students'] as List? ?? [])) {
          ...
          final needs = (summary['needs_practice'] as num?)?.toInt() ?? 0;
          final mastering = (summary['mastered'] as num?)?.toInt() ?? 0;
          states[m['student_id'].toString()] = needs > 0
              ? 'needs_practice'
              : (mastering > 0 ? 'developing' : 'not_started');
        }
        return ListView.separated(
          padding: const EdgeInsets.all(SahlhaSpacing.page),
          itemCount: list.length,
          separatorBuilder: (_, _) => const SizedBox(height: SahlhaSpacing.sm),
          itemBuilder: (_, i) {
            final s = list[i];
            return SahlhaCard(
              onTap: () =>
                  context.push('/teacher/students/$classroomId/${s.id}'),
              ...
              child: Row(
                children: [
                  ...
                  SkillStatusBadge(state: states[s.id] ?? 'not_started'),
                ],
              ),
            );
          },
...
class _MaterialsTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final text = Theme.of(context).textTheme;
    final materials = ref.watch(materialListProvider(classroomId: classroomId));
    return materials.when(
      loading: () => const LoadingState(),
      error: (e, _) => ErrorState(
        message: e.toString(),
        onRetry: () =>
            ref.invalidate(materialListProvider(classroomId: classroomId)),
      ),
      data: (list) => ListView(
        padding: const EdgeInsets.all(SahlhaSpacing.page),
        children: [
          ...
          SahlhaPrimaryButton(
            label: 'Add material',
            onPressed: () =>
                context.push('/teacher/materials/new?classroomId=$classroomId'),
          ),
          const SizedBox(height: SahlhaSpacing.lg),
          if (list.isEmpty)
            const EmptyState(
              title: 'No materials yet',
              message: 'Upload your curriculum (PDF, DOCX, PPTX, TXT, images). Sahlha will find the skills and draft practice.',
            )
          else
            ...list.map(
              (m) => Padding(
...
class _MaterialCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final text = Theme.of(context).textTheme;
    return SahlhaCard(
      onTap: () => context.push('/teacher/materials/${material.id}'),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              Expanded(child: Text(material.title, style: text.titleMedium)),
        ...
        return ListView(
          ...
          children: [
            ...
            ...skills.map((s) {
              ...
              final rate = (skill['mastery_rate'] as num?)?.toDouble();
              return Padding(
                padding: const EdgeInsets.only(bottom: SahlhaSpacing.sm),
                child: SahlhaCard(
                  child: Column(
